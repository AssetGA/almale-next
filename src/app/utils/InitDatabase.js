...
const ProductTranslate = require("../../models/ProductTranslate.js");
...
module.exports = async () => {
  ...
  if (productTranslate.length !== productTranslateMock.length) {
    ...
    await createInitialEntityNew2(
      ProductTranslate,
      productTranslateMock,
      "product",
      list
    );
  }
...
async function createInitialEntityNew2(Model, data, str, list) {
  return await Promise.all(
    data.map(async (item, index) => {
      try {
        delete item._id;
        // каждые 3 элемента data берут один элемент из list
        const listItem = list[Math.floor(index / 3)];
        const newItem = new Model({ ...item, [str]: listItem });
        await newItem.save();
        return newItem;
      } catch (e) {
        console.error(`Error saving item ${index}:`, e);
        return e;
      }
    })
  );
